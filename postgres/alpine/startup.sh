...
init_pg() {
    ...
    su - ${PGUSER} -c "$PGHOME/bin/initdb -D ${PGDATA} -E UTF-8 --user=${PGUSER} --pwfile=\${HOME}/POSTGRES_PASSWORD" || exit $?
...
start_pg() {
    ...
    POSTGRES_HOME=/home/${PGUSER}
    if [ ! -d ${POSTGRES_HOME} ]; then
        usermod -m -d ${POSTGRES_HOME} ${PGUSER}

        if [ ! -d ${POSTGRES_HOME} ]; then
            mkdir -p ${POSTGRES_HOME}
            chown -R ${PGUSER} ${PGGROUP} ${POSTGRES_HOME}
        fi
    fi
    ...
    # # 执行配置
    # if [ "${PGTYPE}" == "MASTER" -a ${ENABLE_CITUS} == "YES" ]; then
